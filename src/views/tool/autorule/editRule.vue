      ...
      >
        ...
        <div class="temp-item-block">
          ...
          <div class="temp-item-main">
            <el-form-item label="账户名" prop="idt_id">
              <el-select
                v-model="form.idt_id"
                :disabled="isEditMode"
                placeholder="账户名"
            ...
            <el-form-item
              label="类别详情"
              prop="rule_obj_id"
              v-show="form.rule_type"
            >
              <el-select
                v-model="form.rule_obj_id"
        ...
        <div class="temp-item-block">
          ...
          <div class="temp-item-main">
            ...
            <!--<el-checkbox v-for="(value,key) in defaultParams.notice_way"-->
            <!--:key="key"-->
            <!--:label="key">{{value}}</el-checkbox>-->
            <!--</el-checkbox-group>-->
            <!--</el-form-item>-->

            <!--<el-form-item label="接收通知者"-->
            <!--prop="receiver_ids">-->
            <!--<el-checkbox-group v-model="form.receiver_ids">-->
            <!--<el-checkbox v-for="(item) in defaultParams.receivers"-->
            <!--:key="item.id"-->
  ...
  watch: {
    ...
    "form.rule_type"(value) {
      ...
      if (this.isEditMode === false) {
        Object.assign(this.form, { rule_obj_id: "", rule_action: "" });
      }
    },
    ...
    onSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (
            postData.rule_action === "4" &&
            budget.opt === "2" &&
            budget.value < 100
          ) {
            this.$message.error("最低预算100元,请调整");
            return;
          }
