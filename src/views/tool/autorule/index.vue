    ...
    <div class="page-content">
      <div class="mg-filter-wrapper">
        ...
        <div class="mg-action-group" style="position:absolute; right: 32px;">
          <router-link to="/tool/autorule/editLoss?rule_id=0">
      ...
      <div class="mg-table-wrapper">
        ...
        >
          ...
          <el-table-column prop="rule_action" label="操作方式">
            <template slot-scope="scope">
              <span v-if="scope.row.rule_action === '10'">直接关停</span>
              <span v-else-if="scope.row.rule_action === '11'"
                >先预警再关停</span
              >
              <span v-else>延期并通知</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEditClick(rule_id) {
      this.$router.push(`/tool/autorule/editLoss?rule_id=${rule_id}`);
    },
