    ...
    >
      <section class="mg-block">
        ...
        <el-form-item label="应用平台" prop="platform">
          <el-radio-group v-model="ruleForm.platform" disabled size="small">
            <el-radio-button
...
export default {
  ...
  created() {
    ...
    this.tabData = { label: "编辑应用组", route: "/putin/appgroup" };
  },
  mounted() {
    this.getConfig();
    this.appGroupDetail();
  ...
  methods: {
    getConfig() {
      this.axios.get("/Admin/Spm_App/getConfig").then(res => {
        if (res.code === 0) {
    ...
    appGroupDetail() {
      ...
      this.axios
        .get("/Admin/Spm_App/appGroupDetail", { params })
        .then(res => {
          if (res.code === 0) {
            let {
              name,
