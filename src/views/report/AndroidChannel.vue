...
export default {
  ...
  created() {
    ...
    this.disabledIndicator = {
      ...
      // active_cost: true,
      // register_cost: true,
      // user_new_pay_cost: true,
      // new_user_pay_rate: true,
      // ltv7: true,
      // roi7: true
    };
  ...
  methods: {
    getConfig() {
      return this.$axios
        .get("/Admin/Spm_Report/getChartConfig?report_type=buy")
        .then(res => {
          if (res.code === 0) {
            let cacheKey = `channelDim_${this.account.user_id}`;
            let cacheDimData =
    ...
    handleExport() {
      ...
      link.setAttribute("download", "安卓渠道报表-" + fileName + ".csv");
